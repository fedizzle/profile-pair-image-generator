...
function middleStripSvg(theme, percent) {
  ...
  const topMark = percent === 69 ? "FIRE" : percent === 110 ? "MAX" : "";
  ...
  return Buffer.from(`
    <svg width="${WIDTH}" height="${HEIGHT}" viewBox="0 0 ${WIDTH} ${HEIGHT}" xmlns="http://www.w3.org/2000/svg">
      ...
      ${isMystery ? `
        <text x="${STRIP_X + 36}" y="119" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="27" font-weight="900" fill="#ffffff">VS</text>
      ` : `
        ...
        <text x="${STRIP_X + 36}" y="61" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="14" fill="#bdbdbd">75</text>
        <text x="${STRIP_X + 36}" y="115" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="14" fill="#bdbdbd">50</text>
        <text x="${STRIP_X + 36}" y="28" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="11" font-weight="800" fill="${theme.accent}">${topMark}</text>
      `}

      ${showPercent ? `
        <rect x="${STRIP_X}" y="158" width="${STRIP_WIDTH}" height="62" fill="${theme.accent}"/>
        <text x="${STRIP_X + 36}" y="199" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="30" font-weight="900" fill="${theme.text}">${percentText}</text>
      ` : ""}
    </svg>
  `);
}
